...
class Fraction:
    ...
    def __init__(self,num=0,denom=1):
        assert type(num) == int and type(denom) == int, 'types given not int'
        assert denom != 0, 'denom cannot be 0'
        
        if denom < 0:
            num = -num
        gcd = self._gcd(abs(num), abs(denom))
        self.num = num // gcd
    ...
    def __bool__(self):
        return not (self.num == 0 and self.denom == 1)
    

    def __getitem__(self,i):
        if i == 0 or 'numerator'.find(str(i)) == 0:
            return self.num
        elif i == 1 or 'denominator'.find(str(i)) == 0:
            return self.denom
        raise TypeError
    ...
    def __sub__(self,right):
        Fraction._validate_arithmetic(right, {Fraction,int},'-',
                                      'Fraction', type_as_str(right))
        return Fraction.__add__(self, -right)
    ...
    def __mul__(self,right):
        ...
        if type(right) == int:
            right = Fraction(right)
        return Fraction(right.num*self.num, right.denom*self.denom)
    ...
    def __pow__(self,right):
        Fraction._validate_arithmetic(right, {int},'**',
                                     'Fraction', type_as_str(right)) 
        if right < 0:
    ...
    def __gt__(self,right):
        ...
        if type(right) == int:
            right = Fraction(right)
        return Fraction.__lt__(right, self)
...
if __name__ == '__main__':
    ...
    driver.default_file_name = 'bscq31W18.txt'
