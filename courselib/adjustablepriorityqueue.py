...
class PriorityQueue:
    """
    Implements an adjustable Priority Queue data type: values are removed
      according to the highest priority value first rule.
    The priority of any value in the PQ can be updated
    """
    def __init__(self,initial_contents=[], key= (lambda x: x), reverse=False):
        """
        ...
        """
        self._values  = [v for v in initial_contents]
        self._lookup  = {v:index for index,v in enumerate(self._values,0)}
        self._key     = key
    ...
    def _swap(self,i,j):
        """
        Swap the values at indexes i and j; but first swap _lookup for keys at these indexes
        """
        self._lookup[self._values[i]], self._lookup[self._values[j]] = j, i
    ...
    def add(self,v):
        ...
        """
        assert v not in self._lookup, 'added existing value('+str(v)+') to priority queue'
        self._lookup[v] = len(self._values)
    ...
    def remove(self):
        ...
        del self._lookup[answer]
        self._values[0] = self._values[-1]
        self._lookup[self._values[0]] = 0
        self._values.pop()                  #cannot store into empty (after pop) list
        self._percolate_down(0)
        return answer

    def updated(self,v):
        assert v in self._lookup, 'updated specified value('+str(v)+') not in priority queue'
        i = self._lookup[v]
        if i > 0 and self._trichotomy(self._values[i] , self._values[self._parent(i)]) > 0:
    ...
    def merge(self,other):
        ...
        """
        for i,v in enum(other._values,len(self._values)):
            self._values.append(v)
            self._lookup[v] = i
        self._heapify()
        other.clear()
    ...
    def __str__(self):
        ...
        """
        return str(self._values)+'   .....   '+str(self._lookup)
    ...
    def __iter__(self):
        ...
        """
        copy = PriorityQueue(contents=self._values,key=self._key,reverse=self._reverse)
...
Command""" 
    ...
    while True:
        ...
        try:
            ...
            elif action == 'c': pq.clear(); priorities.clear()
