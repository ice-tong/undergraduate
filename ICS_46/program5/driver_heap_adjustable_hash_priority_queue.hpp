#ifndef DRIVER_ADJUSTABLE_HASH_PRIORITYQUEUE_HPP_
#define DRIVER_ADJUSTABLE_HASH_PRIORITYQUEUE_HPP_
...
typedef ics::HeapAdjustableHashPriorityQueue<std::string> PriorityQueueType;

class DriverPriorityQueue {
  public:
  DriverPriorityQueue() : q(regular_gt,.65,hash_string){
//std::cout << "DriverPriorityQueue constructor started" << std::endl;
      ...
      q = PriorityQueueType(gt,.75,hash_string);
      process_commands("");
    };

  private:
    PriorityQueueType q;//{regular_gt,.85,hash_string};
    ...
    std::string menu_prompt (std::string preface) {
      std::cout << std::endl << std::endl << preface+"priority queue q = " << q.str() << std::endl;
      ...
      std::cout << preface+"  u  - update         <  - <<                q  - quit"              << std::endl;
      std::cout << preface+"  x  - clear          r  - relations" << std::endl;
      std::cout << preface+"  =  - =" << std::endl;

      std::string allowable[] = {"e","E","d","u","x","=","m","s","p","<","r","lf","l{","it","q",""};
    ...
    void process_commands(std::string preface) {
      for (;;) try {
        ...
        if (command == "e") {
          ...
          std::string old_value = ics::prompt_string("  Enter value in priority queue ");
          std::string new_value = ics::prompt_string("  Enter value to replace it with");
          q.update(old_value,new_value);
        }
...
#endif /* DRIVER_ADJUSTABLE_HASH_PRIORITYQUEUE_HPP_ */
