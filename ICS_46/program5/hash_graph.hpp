...
HashGraph<T>::HashGraph (const HashGraph& g) {
    ...
    for (NodeMapEntry &node : node_values)
        node.second.connect(this);
}
...
T HashGraph<T>::edge_value(NodeName origin, NodeName destination) const {
    if (!has_edge(origin,destination))
        throw GraphError("HashGraph::edge_value edge not in graph");
...
int HashGraph<T>::in_degree(NodeName node_name) const {
    if (!has_node(node_name))
        throw GraphError("HashGraph::in_degree node not in graph");
...
int HashGraph<T>::out_degree(NodeName node_name) const {
    if (!has_node(node_name))
        throw GraphError("HashGraph::out_degree node not in graph");
...
auto HashGraph<T>::out_nodes(NodeName node_name) const -> const NodeSet& {
    if (!has_node(node_name))
        throw GraphError("HashGraph::out_nodes node not in graph");
...
auto HashGraph<T>::in_nodes(NodeName node_name) const -> const NodeSet& {
    if (!has_node(node_name))
        throw GraphError("HashGraph::in_nodes node not in graph");
...
auto HashGraph<T>::out_edges(NodeName node_name) const -> const EdgeSet& {
    if (!has_node(node_name))
        throw GraphError("HashGraph::out_edges node not in graph");
...
auto HashGraph<T>::in_edges(NodeName node_name) const -> const EdgeSet& {
    if (!has_node(node_name))
        throw GraphError("HashGraph::in_edges node not in graph");
...
void HashGraph<T>::remove_node (NodeName node_name){
    ...
    EdgeSet ies(node_values[node_name].in_edges);
    EdgeSet oes(node_values[node_name].out_edges);
    for (Edge &e : ies)
...
void HashGraph<T>::load (std::ifstream& in_file, std::string separator) {
    ...
    while (getline(in_file,line)) {
        std::vector<std::string> sp_line = ics::split(line, separator);
        if (sp_line.size() != 3)
            add_node(sp_line[0]);
        else {
            std::istringstream iss(sp_line[2]);
            int i = 0;
            iss >> i;
            add_edge(sp_line[0], sp_line[1], i);
        }
    }
}
...
void HashGraph<T>::store(std::ofstream& out_file, std::string separator) {
    ...
    for (EdgeMapEntry &edge: edge_values) {
        out_file << edge.first.first << separator;
        out_file << edge.first.second << separator;
        out_file << edge.second << std::endl;
    }
}
...
std::ostream& operator<<(std::ostream& outs, const HashGraph<T>& g) {
    outs << "graph[" << std::endl;
    HeapPriorityQueue<typename HashGraph<T>::NodeLocalEntry> pq(HashGraph<T>::LocalInfo_gt);
    pq.enqueue_all(g.node_values);
    for (auto &node : pq)
        outs << node.first << " -> " << node.second << std::endl;
    outs << "]";
    return outs;
}
