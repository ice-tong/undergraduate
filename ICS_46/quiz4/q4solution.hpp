...
bool is_bst(TPN<T>* root) {
    if (root == nullptr)
        ...
        return (all_less(root->left, root->value) && all_greater(root->right, root->value))
                ? is_bst(root->left) && is_bst(root->right) : false;
}
...
TPN<T>* bt_to_dlcl(TPN<T>* root) {
    if (root == nullptr)
        ...
        TPN<T>* left = bt_to_dlcl(root->left);
        TPN<T>* right = bt_to_dlcl(root->right);
        root->left = root;
        root->right = root;
