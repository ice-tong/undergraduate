...
template<class T, int (*thash) (const T& a)> class HashEquivalence {
  ...
  private:
    ...
    HashMap<int,int,hash_int> histogram;
    int max_set_size = 0;
    int calls = 0;
...
T HashEquivalence<T,thash>::compress_to_root (T a) {
  ...
  if (compress_set.size() > max_set_size)
      max_set_size = compress_set.size();
...
std::string HashEquivalence<T,thash>::equivalence_info () const {
  std::ostringstream answer;
  answer << "Max size of compress set: " << max_set_size << std::endl;
  answer << "map[size of compress set -> number of sets with that size]: " << histogram << std::endl;
  answer << "Total calls to compress_to_root: " << calls << std::endl;
  ...
  if (calls != 0) {
      ...
      average_set_size = double(sum)/calls;
  }
  answer << "Average size of compress set: " << average_set_size << std::endl;
