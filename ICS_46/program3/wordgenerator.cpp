...
typedef ics::BSTMap<WordQueue,FollowSet>   Corpus;
...
typedef ics::HeapPriorityQueue<CorpusEntry, CorpusEntry_gt> CorpusPQ; // decided to provide gt in template
...
void print_corpus(const Corpus& corpus) {
  ...
  CorpusPQ words_sorted(corpus);
  s_sort.stop();
  int min = std::numeric_limits<int>::max(), max = 0;
  for (const auto& kv : words_sorted) {
...
//  read_corpus time                   = 103.687 seconds
//  print_corpus (sort not print) time = 1468.61 seconds

//Almost All Array Implementations (except HeapPriorityQueue and BSTMap)
//  read_corpus time                   = 1.641 seconds
//  print_corpus (sort not print) time = 0.328 seconds
