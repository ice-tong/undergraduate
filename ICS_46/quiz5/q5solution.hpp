...
std::string longest_word (const DTN& root) {
    std::string result = "";
    if (root.is_word && root.word_to_here.size() > result.size())
        result = root.word_to_here;
    for (const auto& kv : root.children) {
        std::string longest_child = longest_word(kv.second);
        if (longest_child.size() > result.size())
            result = longest_child;
    }
...
ics::pair<int,int> stats (const DTN& root) {
    ...
    if (root.is_word)
        ++words;
    for (const auto& kv : root.children) {
        ics::pair<int,int> child_stats = stats(kv.second);
        nodes += child_stats.first;
        words += child_stats.second;
    }

    return ics::make_pair(nodes,words);
}
