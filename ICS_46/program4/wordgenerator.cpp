...
int hash_queue(const WordQueue& q) {
  int hash = 0;
  for (auto s : q)
    hash += hash_str(s);
  return hash;
...
//  read_corpus time                   = 0.312
//  print_corpus (sort not print) time = 0.11

//Array Implementations for Queue; HeapPriorityQueue, HashMap, and HashSet
//  read_corpus time                   = 0.375
//  print_corpus (sort not print) time = 0.187

//Briefly explain (in a comment below) what is interesting
//about the times above (and explain why it occurs).

/*
 * Generally, a HashSet implementation would be faster than an ArraySet implementation.
 * However, this is only true if we consider ALL of the things we can do with
 * sets (e.g. insert, contains, erase, etc.). HashSets are especially faster at finding values
 * than ArraySets.  However, in this word generator program, we NEVER actually search for values in
 * the sets.  We only insert into the sets in read_corpus.  This is why the times when using HashSet
 * are actually a bit slower than when using ArraySet.  ArraySet only has to insert onto the end of the
 * array while HashSet has to calculate the hash code, then create a new LN, then insert into the proper bin.
 * HashSet also has to resize AND rehash every time we insert past the LF, while ArraySet just has to resize
 * an array and not worry about rehashing or creating new trailer nodes.  If this program utilized the set
 * operations besides insert, it's very likely that the HashSet implementation would be faster.
 */
