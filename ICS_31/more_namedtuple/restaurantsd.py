#  Kian Farsany 55375951 and Carina Javalera 51817757.  ICS 31 Lab sec 8.  Lab asst 5d.
...
def Restaurant_change_price(r: Restaurant, n: float) -> Restaurant:
    '''Return the same Restaurant with the price changed by the given percentage'''
    if n <= -100:
        return Restaurant(r.name, r.cuisine, r.phone, r.dish, 0)
    if n > -100 and n < 0:
        return Restaurant(r.name, r.cuisine, r.phone, r.dish, r.price * (1-(abs(n)/100)))
    if n > 0 and n < 100:
        return Restaurant(r.name, r.cuisine, r.phone, r.dish, r.price + (r.price * (n/100)))
    if n >= 100:
        return Restaurant(r.name, r.cuisine, r.phone, r.dish, r.price * (n/50))
    return r
