#  Kian Farsany 55375951 and Andrew Severt 27644841.  ICS 31 Lab sec 8.  Lab asst 8e.
##
...
def Song_str(S: Song) -> str:
    '''Format a Song into a readable string'''
    return "Track {}:  '{}'     Length: {} seconds     Played {} time(s)".format(S.track, S.title, S.length, S.play_count)

def Song_listening_time(S: Song) -> int:
    '''Calculates the total listening time for one Song'''
    return S.length * S.play_count

def Album_str(A: Album) -> str:
    '''Format an Album into a readable string'''
    result = "Album No. {}:  '{}'\nBy {}, {}\nSongs:\n".format(A.id, A.title, A.artist, A.year)
    for S in A.songs:
        result = result + Song_str(S) + '\n'
    return result

def Album_listening_time(A: Album) -> int:
    '''Return the total listening time for an Album of Songs'''
    result = 0
    for S in A.songs:
...
def Album_play_count(A: Album) -> int:
    '''Return the play count for all of the Songs in the Album'''
    result = 0
    for S in A.songs:
        result += S.play_count
    return result
...
def favorite_album(MC: [Album]) -> Album:
    '''Return the Album with the longest listening time'''
...
def favorite_album2(MC: [Album], keyfunction: 'Function on Albums') -> Album:
    '''Return the favorite album according to the keyfunction'''
    MC.sort(key=keyfunction)
    return MC[-1]
...
def Album_tracks(A: Album) -> int:
    '''Return the number of tracks in an Album'''
    return len(A.songs)
...
def Songdisplay_str(SD: Songdisplay) -> str:
    '''Format a Songdisplay into a readable string'''
    return "Track {}: '{}'   Length: {} seconds   Played {} times\nAlbum: {}   By {}, {}".format(SD.track, SD.s_title, 
                                                                                                 SD.length, SD.play_count, SD.a_title,
                                                                                                 SD.artist, SD.year)
def length_from_Songdisplay(SD: Songdisplay) -> int:
    '''Return the length of the Song from the Songdisplay'''
    return SD.length
...
def unplayed_songs(MC: [Album]) -> [Songdisplay]:
    '''Return the songs that have never been played'''
    result = []
...
def top_n(MC: [Album], n: int, keyfunction: 'Function on Albums', b: bool) -> [Songdisplay]:
    '''Return the highest or lowest n songs in MC according to the keyfunction and boolean'''
    list_of_Songdisplays = all_Songdisplays(MC)
    list_of_Songdisplays.sort(key=keyfunction, reverse=b)
    return list_of_Songdisplays[:n]

def collection_search(MC: [Album], s: str) -> [Songdisplay]:
    '''Return a list of Songdisplays with the given string'''
    result = []
    list_of_Songdisplays = all_Songdisplays(MC)
